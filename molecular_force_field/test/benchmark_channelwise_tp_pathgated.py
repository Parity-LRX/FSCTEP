...
def _expand_path_gates(gates: torch.Tensor, num_paths: int, weight_numel: int) -> torch.Tensor:
    if weight_numel % num_paths != 0:
        raise ValueError(f"weight_numel={weight_numel} not divisible by num_paths={num_paths}")
    block = weight_numel // num_paths
    return gates.repeat_interleave(block, dim=-1)
...
def bench_e3nn(batch: int, mul: int, lmax: int, device: torch.device, dtype: torch.dtype, warmup: int, repeat: int):
    ...
    gates = torch.randn(batch, len(instructions), device=device, dtype=dtype, requires_grad=True)

    def run():
        zero_dense_grads(mod, x1, x2, gates)
        w = _expand_path_gates(gates, len(instructions), mod.weight_numel)
        loss = loss_from_output(mod(x1, x2, w))
        loss.backward()

    return measure(run, warmup=warmup, repeat=repeat, device=device), len(instructions)
...
def bench_cueq(batch: int, mul: int, lmax: int, device: torch.device, dtype: torch.dtype, warmup: int, repeat: int):
    ...
    # ChannelWiseTP block size equals mul_in1 * mul_in2 = mul for this workload.
    num_paths = mod.weight_numel // mul
    gates = torch.randn(batch, num_paths, device=device, dtype=dtype, requires_grad=True)

    def run():
        zero_dense_grads(mod, x1, x2, gates)
        w = _expand_path_gates(gates, num_paths, mod.weight_numel)
        loss = loss_from_output(mod(x1, x2, w))
        loss.backward()

    return measure(run, warmup=warmup, repeat=repeat, device=device), num_paths
...
def plot_results(rows: list[dict], out_png: Path):
    ...
    series = [
        ("e3nn", "e3nn path-gated"),
        ("cueq", "cueq path-gated"),
        ("ictd", "ictd path-gated"),
        ("ictd_o3", "ictd-o3 path-gated"),
    ]
    ...
    ax_speed.set_title("Path-Gated Conv TP Speed")
    ax_mem.set_title("Path-Gated Conv TP Memory")
    ...
    ax_speed.legend()
    ax_mem.legend()
    fig.suptitle("Path-Gated Conv-style TP Backends in FP32 on GPU")
    fig.tight_layout()
...
def main():
    ...
    p.add_argument("--out-json", default="/home/rebuild/.bench_channelwise_tp_pathgated.jsonl")
    p.add_argument("--out-png", default="/home/rebuild/channelwise_tp_pathgated.png")
    args = p.parse_args()
    ...
    for lmax in range(args.lmax_max + 1):
        (e3nn_ms, e3nn_mb), e3nn_num_paths = bench_e3nn(args.batch, args.mul, lmax, device, dtype, args.warmup, args.repeat)
        (cueq_ms, cueq_mb), cueq_num_paths = bench_cueq(args.batch, args.mul, lmax, device, dtype, args.warmup, args.repeat)
        ...
        row = {
            ...
            "e3nn_ms": e3nn_ms,
            "e3nn_mb": e3nn_mb,
            "e3nn_num_paths": e3nn_num_paths,
            "cueq_ms": cueq_ms,
            "cueq_mb": cueq_mb,
            "cueq_num_paths": cueq_num_paths,
            "ictd_ms": ictd_ms,
            "ictd_mb": ictd_mb,
