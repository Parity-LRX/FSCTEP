...
class ICTDMACEUSymmetricContractionSO3(nn.Module):
    """
    MACE U_matrix_real symmetric contraction expressed directly in the ICTD basis.
    ...
    def _transform_u_tensor(self, u_tensor: torch.Tensor, output_l: int) -> torch.Tensor:
        ...
        raise NotImplementedError(f"ICTD MACE-U contraction currently supports correlation<=3, got nu={nu}")
...
class ICTDMACEUProductBasisBlockSO3(nn.Module):
    def __init__(
        self,
        *,
        num_elements: int,
        ...
        self.target_lmax = int(target_lmax)
        self.target_irreps = _hidden_irreps(self.channels, self.target_lmax)
        self.symmetric_contractions = ICTDMACEUSymmetricContractionSO3(
            num_elements=num_elements,
            channels=channels,
            lmax=lmax,
    ...
    def forward(self, node_feats: torch.Tensor, sc: torch.Tensor | None, node_attrs: torch.Tensor) -> torch.Tensor:
        ...
        if sc is not None:
            if sc.shape[-1] == out.shape[-1]:
                ...
                raise ValueError(f"Cannot add sc shape {tuple(sc.shape)} to MACE-U output {tuple(out.shape)}")
        return out
...
class ICTDPureUSymmetricContractionSO3(nn.Module):
    ...
    pure-ICTD contraction ablation against `ictd-mace-u`.
    """
...
class PureCartesianICTDFix(nn.Module):
    ...
    def __init__(
        ...
        if ictd_fix_product_backend not in {"ictd", "native-mace", "ictd-mace-u", "ictd-pure-u"}:
            raise ValueError(
                "ictd_fix_product_backend must be 'ictd', 'native-mace', 'ictd-mace-u', "
                f"or 'ictd-pure-u', got {ictd_fix_product_backend!r}"
            )
        ...
        for layer_idx, target_lmax in enumerate(product_target_lmax):
            ...
            if self.ictd_fix_product_backend == "native-mace":
            ...
            elif self.ictd_fix_product_backend == "ictd-mace-u":
                self.products.append(
                    ICTDMACEUProductBasisBlockSO3(
                        num_elements=self.num_elements,
                        channels=self.channels,
                        lmax=self.lmax,
                        target_lmax=target_lmax,
            ...
            elif self.ictd_fix_product_backend == "ictd-pure-u":
                self.products.append(
        ...
        if self.ictd_fix_route == "fusion":
            ...
            self.multiple_contraction_mix = ICTDPureUFusionMixBlockSO3(
                num_elements=self.num_elements,
